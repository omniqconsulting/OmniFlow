...
def upgrade() -> None:
    ...
    op.add_column('fms_stages', sa.Column('split_enabled', sa.Boolean(), nullable=True, server_default=sa.false()))
    op.add_column('fms_stages', sa.Column('split_target_field', sa.String(), nullable=True))
    op.add_column('fms_stages', sa.Column('split_actual_field', sa.String(), nullable=True))

    # --- library_flow_stages: mirror config on library template stages ---
    op.add_column('library_flow_stages', sa.Column('split_enabled', sa.Boolean(), nullable=True, server_default=sa.false()))
    op.add_column('library_flow_stages', sa.Column('split_target_field', sa.String(), nullable=True))
    op.add_column('library_flow_stages', sa.Column('split_actual_field', sa.String(), nullable=True))

    # --- fms_ticket_splits: hierarchical split-record fields (additive) ---
    op.add_column('fms_ticket_splits', sa.Column('root_ticket_id', sa.String(), nullable=True))
    op.add_column('fms_ticket_splits', sa.Column('split_display_id', sa.String(), nullable=True))
    op.add_column('fms_ticket_splits', sa.Column('split_sequence', sa.Integer(), nullable=True))
    op.add_column('fms_ticket_splits', sa.Column('split_stage_id', sa.String(), nullable=True))
    op.add_column('fms_ticket_splits', sa.Column('target_value_at_split', sa.Float(), nullable=True))
    op.add_column('fms_ticket_splits', sa.Column('entered_value', sa.Float(), nullable=True))
    op.add_column('fms_ticket_splits', sa.Column('is_remainder', sa.Boolean(), nullable=True, server_default=sa.false()))
    op.add_column('fms_ticket_splits', sa.Column('is_auto_split', sa.Boolean(), nullable=True, server_default=sa.false()))

    with op.batch_alter_table('fms_ticket_splits') as batch_op:
        batch_op.create_foreign_key(
            'fk_fms_ticket_splits_root_ticket_id', 'fms_tickets',
            ['root_ticket_id'], ['id']
        )
        batch_op.create_foreign_key(
            'fk_fms_ticket_splits_split_stage_id', 'fms_stages',
            ['split_stage_id'], ['id']
        )
